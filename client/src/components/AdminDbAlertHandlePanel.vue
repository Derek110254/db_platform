...
/**
 * 表格容器：fixed 布局 + 百分比列宽已保证表格严格等于容器宽度，
 * 用 hidden 根除亚像素溢出导致的滚动条
 */
.table-wrap {
  width: 100%;
  overflow-x: hidden;
...
/**
 * 关键修复：fixed 布局 + colgroup 百分比列宽，防止长文本撑开列导致重叠
 * box-sizing: border-box 确保 border 不额外撑宽表格
 */
.result-table {
  width: 100%;
  max-width: 100%;
...
/**
 * 长文本列允许换行（fixed 布局下列宽已锁定，内容自然换行）
 */
.cell-wrap {
  white-space: normal;
}
